...
import { NotesClient, QuickCaptureRequest, NoteListItemResponse, API_BASE_URL } from '../api/client';
...
export function useQuickCapture() {
  ...
  return useMutation({
    mutationFn: async (data: QuickCaptureRequest) => {
      const client = getNotesClient();
      return client.quickCapture(data);
    },
    onSuccess: () => {
...
export function useRecentCaptures(limit: number = 5) {
  return useQuery({
    ...
    select: (data): Array<{
      ...
      data.map((note: NoteListItemResponse) => ({
        id: note.id ?? '',
        title: note.title ?? '',
...
export function useRecentTags(limit: number = 10) {
  return useQuery({
    ...
    select: (data: NoteListItemResponse[]) => {
